...
int main() {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      for (int i = 0; i < 4; i++) {
        if (i == 0)
          ...
          tmp1[i][j] = gMulBy3[v[0]] ^ v[1] ^ v[2] ^ gMulBy2[v[3]];
      }
    }
  ...
  vector<int> temp1;
  for (int i = 0; i < 4; i++) {
    temp1.push_back(key[i][3]);
  }
  int arr1[4];
  for (int i = 0; i < 4; i++) {
    arr1[i] = temp1[(i + 1 + 4) % 4];
  }
